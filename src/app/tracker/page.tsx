...
export default function TrackerPage() {
  ...
  const updateProgress = useCallback(
    ...
    [supabase, currentMonth, totalDays]
  );
  ...
  const toggleDaily = async (task: Task, checked: boolean) => {
    if (checked) {
      ...
      await supabase.from("task_logs").delete().eq("task_id", task.id).eq("date", todayStr);
    }
  ...
  const addTask = async (type: "routine" | "daily", name: string) => {
    ...
    await supabase.from("tasks").upsert(
      ...
      { onConflict: "user_id,task_name,month_year" }
    );
  ...
  // --- FINISHED DAILY TASKS (today only) ---
  const finishedToday = dailyTasks.filter((task) =>
    taskLogs.some((log) => log.task_id === task.id && log.date === todayStr && log.completed)
  );
            ...
            <div className="flex flex-col gap-4">
              {routineTasks.map((t) => {
                ...
                return (
                  <div
                    key={t.id}
                    className="border border-border rounded-2xl p-4 bg-surface/60 shadow-sm transition hover:shadow-md relative"
                  >
                    {/* Delete button */}
                    <button
                      onClick={() => deleteTask(t.id)}
                      className="absolute top-1 right-1 text-foreground/50 hover:text-foreground transition"
                    >
                      <X size={16} />
                    </button>
                    ...
                    <div className="flex justify-between items-center mb-2">
                      ...
                      <button
                        onClick={() => markToday(t)}
                        className="px-3 py-1 text-sm rounded-full bg-foreground text-background hover:opacity-80 transition"
                      >
                        Mark Today
                      </button>
                    </div>
        ...
        <section>
          ...
          {unfinishedToday.length === 0 ? (
            ...
            <div className="flex flex-col gap-3">
              {unfinishedToday.map((t) => (
                <label
                  key={t.id}
                  className="flex items-center gap-3 bg-surface/60 border border-border rounded-xl p-3 cursor-pointer hover:bg-surface/80 transition relative"
                >
                  <input
                    type="checkbox"
                    checked={false}
                  ...
                  <button
                    onClick={() => deleteTask(t.id)}
                    className="absolute right-3 text-foreground/50 hover:text-foreground transition"
                  >
                    <X size={16} />
                  </button>
                </label>
          ...
          <div className="mt-8">
            ...
            {finishedToday.length === 0 ? (
              <p className="text-sm text-foreground/60">No tasks finished yet today.</p>
            ) : (
              <div className="flex flex-col gap-2">
                {finishedToday.map((t) => (
                  <div
                    key={t.id}
                    className="flex items-center justify-between bg-foreground text-background rounded-lg px-4 py-2 transition hover:opacity-90"
                  >
